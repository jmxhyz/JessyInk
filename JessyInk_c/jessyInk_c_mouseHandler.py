...
class	JessyInk_CustomMouseHandler(inkex.Effect):
	...
	def effect(self):
		...
		if self.options.mouseSettings == "noclick":
			...
			scriptElm.text = open(os.path.join(os.path.dirname(__file__),	"jessyInk_c_core_mouseHandler_noclick.js")).read()
			
			# Check whether a hash bang line is present and if so comment it out.
			if scriptElm.text.find("#!") == 0:
			...
			# Create new script node.
			scriptElm = inkex.etree.Element(inkex.addNS("script", "svg"))
			scriptElm.text = open(os.path.join(os.path.dirname(__file__),	"jessyInk_c_core_mouseHandler_zoomControl.js")).read()
